import numpy as np
from monopoly_simulator import gameplay, action_choices, novelty_functions
...
def play_tournament_with_novelty_1(tournament_log_folder=None, meta_seed=5, num_games=100, novelty_index=23, novelty_info=False):
    ...
    for t in range(novelty_index, len(tournament_seeds)):
        ...
        new_winners.append(gameplay.play_game_in_tournament(tournament_seeds[t], novelty_info, improvePropertyRed_novelty))
        handlers_copy = logger.handlers[:]
        for handler in handlers_copy:
            logger.removeHandler(handler)
            handler.close()
    ...
    for i in range(len(agent_combination_1)):
        ...
        for t in range(novelty_index, len(tournament_seeds)):
            ...
            new_winners.append(gameplay_socket.play_game_in_tournament_socket(tournament_seeds[t], agent, agent_combination_1[i][0], agent_combination_1[i][1],
                                                                              agent_combination_1[i][2], novelty_info, buy_discount_novelty))
            handlers_copy = logger.handlers[:]
            for handler in handlers_copy:
                logger.removeHandler(handler)
                handler.close()
...
def play_tournament_with_novelty_buy_property_right(tournament_log_folder=None, meta_seed=5, num_games=100, novelty_index=23, novelty_info=False):
    """
    Use this function for the normal gameplay without socket communication
    :param tournament_log_folder:
    :param meta_seed:
    ...
    for t in range(novelty_index, len(tournament_seeds)):
        ...
        logger = log_file_create(filename)
        # new_winners.append(gameplay.play_game_in_tournament(tournament_seeds[t], novelty_info, class_novelty_1))
        new_winners.append(gameplay.play_game_in_tournament(tournament_seeds[t], novelty_info, buy_property_right))
        handlers_copy = logger.handlers[:]
        for handler in handlers_copy:
            logger.removeHandler(handler)
            handler.close()
...
def buy_discount_novelty(current_gameboard):
    action_choices.buy_property = novelty_functions.buy_discount_property

# def
def buy_property_right(current_gameboard):
    action_choices.buy_property = novelty_functions.buy_property_right

def buy_property_rightplayer(current_gameboard):
    action_choices.buy_property = novelty_functions.buy_property_rightplayers

def improvePropertyRed_novelty(current_gameboard):
    action_choices.improve_property = novelty_functions.improvePropertyRed_novelty
...
play_tournament_with_novelty_2(tournament_log_folder='/tournament_with_novelty/', meta_seed=15, num_games=2, novelty_index=1, novelty_info=True)
